...
object EnvironmentUtils {
     ...
     * Trusting it there made Shizuku skip discovery and connect to a dead port,
     * failing every start until wireless debugging was toggled once (issue #188).
     *
     * `service.adb.tcp.port` is volatile (cleared on reboot, set only while adbd is
     * actively listening on TCP), so it is a real liveness signal. Non-TLS TVs
     * cannot use mDNS discovery at all, so their configured port stays a valid
     * direct target. Everything else must go through discovery instead of trusting
     * a persisted hint.
     */
